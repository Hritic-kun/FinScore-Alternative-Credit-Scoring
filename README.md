...
## 📌 Project Overview
Traditional credit scoring relies heavily on   scores, often excluding "new-to-credit" or "thin-file" segments. This project develops a **Fintech-focused Credit Risk Model** that incorporates alternative data points—such as debt-to-income (DTI) ratios, employment stability, and inquiry frequency—to predict the Probability of Default ($PD$).
...
1. **Feature Engineering:** - Created an `Income_Stability_Index` and `Installment_to_Income` ratio to assess repayment capacity beyond simple salary figures.
   - Developed a `Custom_Risk_Score` blending traditional   data with alternative financial health indicators.
...
- **Business Insights:** Identified "hidden gem" borrowers—users with average   scores but high repayment probability based on stable DTI and employment tenure.
